"""Outbox event writer for reliable event delivery in Expense Management.
...
def write_domain_event(
    ...
    try:
        ...
        elif hasattr(domain_event, 'expense_id'):
            # For expense events
            expense_id = getattr(domain_event, 'expense_id')
            if hasattr(expense_id, 'value'):
                aggregate_id = expense_id.value
        elif hasattr(domain_event, 'category_id'):
            # For category events
            category_id = getattr(domain_event, 'category_id')
            if hasattr(category_id, 'value'):
                aggregate_id = category_id.value
